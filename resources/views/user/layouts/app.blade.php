...
<nav class="navbar navbar-expand-md navbar-light bg-white shadow-sm">
                        ...
                        @else
                            <li class="nav-item dropdown">
                                ...
                                @if (Auth::user()->role == "user" || Auth::user()->role == "admin")
                                    <a class="dropdown-item important-nav" href="{{ route('pemilik.register') }}">Jadi pemilik mobil</a>
                                @else
                                    <a class="dropdown-item important-nav" href="{{ route('pemilik.dashboard') }}">Halaman Pemilik</a>
                                @endif
    ...
    <script type="text/javascript">
                ...
                function (data, textStatus, jqXHR) {  // success callback
                    if(data.length > 0){
                        $.each(data, function(index, element) {
                            ...
                            if(index == 0){
                                $( ".chat-room" ).html(
                                    `<div class="pesan row p-1 user-${data_user['id']}" onclick="show_message('${data[index]['id']}','${data_user['name']}')">
        ...
        function open_message(user_id){
            $.when( show_room() ).done(function() {
